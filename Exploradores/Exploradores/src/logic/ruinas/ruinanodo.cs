...
namespace Ruinas
{
	...
	public class RuinaNodo : Dijkstra.INodo
	{
		...
		public bool colisiona(int x, int y, PersonajeRuina objeto)
		{
			int xI = coordenadas.Item1 - PersonajeRuina.ancho / 2;
			int yI = coordenadas.Item2 - PersonajeRuina.alto / 2;
			int xF = x - PersonajeRuina.ancho / 2;
			...
			bool conditionX = maxX > objeto.posicion.X && minX < objeto.posicion.X + objeto.posicion.Width;
			bool conditionY = maxY > objeto.posicion.Y && minY < objeto.posicion.Y + objeto.posicion.Height;
			...
			if (conditionX)
			{
				...
				int minMedY = medY1 < medY2 ? medY1 : medY2;
				int maxMedY = medY1 > medY2 ? medY1 : medY2;
				if (maxMedY >= objeto.posicion.Y && conditionY)
					return true;
				if (minMedY <= objeto.posicion.Y + objeto.posicion.Height && conditionY)
					return true;
			}
			...
			if (conditionY)
			{
				...
				int minMedX = medX1 < medX2 ? medX1 : medX2;
				int maxMedX = medX1 > medX2 ? medX1 : medX2;
				if (maxMedX >= objeto.posicion.X && conditionX)
					return true;
				if (minMedX <= objeto.posicion.X + objeto.posicion.Width && conditionX)
					return true;
			}
