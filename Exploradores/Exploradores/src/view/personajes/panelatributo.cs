...
namespace Personajes
{
	...
	public class PanelAtributo : InterfazGrafica
	{
		// variables
		...
		// constructor
		public PanelAtributo(bool actualizarVista = true)
			: base()
		{
		...
		public override void updateContent()
		{
			...
			if(Gestores.Partidas.Instancia.atributoSeleciconado == null)
				return;
			...
			Atributo atributo = Gestores.Partidas.Instancia.atributoSeleciconado;
			...
			label = new ILSXNA.Label();
			label.message = "Valor actual: " + atributo.valor.ToString();
			label.color = color;
			label.innerComponent = font;
			contenedor.addComponent(label);

			label = new ILSXNA.Label();
			label.message = "Rango valores: [ " + atributo.valorMin.ToString() + " - " + atributo.valorMax.ToString() + " ]";
			label.color = color;
			label.innerComponent = font;
			contenedor.addComponent(label);

			contenedor.addComponent(atributo.getVistaEspecifica());
			...
			label = new ILSXNA.Label();
			label.message = atributo.descripcion;
			label.color = color;
			label.innerComponent = font;
			...
			boton.updateContent();
			boton.onButtonPress = Programa.Controller.funcionCerrarAtributo;
			addComponent(boton);
		}
	}
}
